...
const SUPABASE_KEY = process.env.VITE_SUPABASE_PUBLISHABLE_KEY; // Anon key is fine for login
...
async function main() {
  // 1. Login with multiple password attempts just in case
  const passwords = ['admin123', 'supabasetest', '123456', 'Admin@123', 'senha123'];
  let session = null;
  ...
  if (!session) {
    console.error("Could not authenticate as admin@sgc.com. Check password.");
    process.exit(1);
  }

  // 2. Call the Copilot Edge Function
  console.log("Calling Copilot edge function...");
  const { data: edgeData, error: edgeError } = await supabase.functions.invoke('copilot', {
    body: {
  ...
  console.log("Success! AI Trained:", edgeData);
}
