...
export interface Modalidade {
  id: string;
  nome: string;
  tipo_documento: 'CPF' | 'CNPJ' | 'EMPRESA';
...
export function useCreateModalidade() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (m: { nome: string; tipo_documento: 'CPF' | 'CNPJ' | 'EMPRESA'; documentos_obrigatorios: string[]; documentos_opcionais: string[]; quantidade_vidas: string }) => {
...
export function useUpdateModalidade() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, ...rest }: { id: string; nome: string; tipo_documento: 'CPF' | 'CNPJ' | 'EMPRESA'; documentos_obrigatorios: string[]; documentos_opcionais: string[]; quantidade_vidas: string }) => {
