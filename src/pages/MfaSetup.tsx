...
const MfaSetup = ({ onVerified }: MfaSetupProps) => {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (trustDevice) {
        // Non-blocking: trusted device insert should never block MFA success
        try {
          const deviceHash = await generateDeviceHash();
          const trustedUntil = getTrusted30Days();
          const { error: trustError } = await supabase.from('mfa_trusted_devices').insert({
            user_id: user!.id,
          ...
          if (!trustError) {
            ...
            console.warn('mfa_trusted_devices insert falhou (não crítico):', trustError);
          }
        } catch (trustErr) {
          console.warn('mfa_trusted_devices exception (não crítico):', trustErr);
        }
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-6">
      <div className="w-full max-w-md space-y-6">
        <div className="text-center">
          <img src={logo} alt="Vitaliza Seguros" className="h-10 mx-auto mb-6" />
